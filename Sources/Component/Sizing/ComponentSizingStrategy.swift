...
public enum ComponentSizingStrategy: Equatable, Sendable {
    ...
    /// и соответствующий приоритет для сопротивления к сжатию в зависимости от флага `forced`.
    ///
    ...
    ///
    /// - Parameter forced: Определяет, может ли компонент расширять размеры контейнера.
    case hug(forced: Bool)
...
extension ComponentSizingStrategy {

    /// Компонент стремится сохранить свой собственный размер, расширяя размеры контейнера.
    ///
    /// Фактически означает, что компонент будет иметь обязательный приоритет для сопротивления
    /// но не будет принудительно расширять размеры контейнера.
    ///
    /// Чтобы система смогла определить собственный размер компонента,
    /// рекомендуется "замыкать" констрейнты в UIKit-представлении по соответствующей оси
    /// или возвращать определенное значение размера в свойстве `intrinsicContentSize`.
