...
internal final class CollectionViewLayoutUpdateManager<Layout: ListLayout> {
    ...
    private func makeItemForReloading(at indexPath: IndexPath, state: ListLayoutState<Layout>) -> ListLayoutItem {
        ListLayoutItem(
            indexPath: indexPath,
    ...
    private func makeItemForInserting() -> ListLayoutItem {
        ListLayoutItem()
    }
    ...
    private func handleReloadAction(of update: UICollectionViewUpdateItem, state: ListLayoutState<Layout>) {
        guard let indexPath = update.indexPathBeforeUpdate else {
            fatalError("`indexPathBeforeUpdate` cannot be `nil` for a `reload` update action")
        }
        ...
        if indexPath.item == NSNotFound {
            ...
            itemsToReload[indexPath] = makeItemForReloading(
                at: indexPath,
                state: state
            )
        }
    ...
    private func handleDeleteAction(of update: UICollectionViewUpdateItem) {
        guard let indexPath = update.indexPathBeforeUpdate else {
            fatalError("`indexPathBeforeUpdate` cannot be `nil` for a `delete` update action")
        }
    ...
    private func handleInsertAction(of update: UICollectionViewUpdateItem) {
        guard let indexPath = update.indexPathAfterUpdate else {
            fatalError("`indexPathAfterUpdate` cannot be `nil` for an `insert` update action")
        }

        if indexPath.item == NSNotFound {
            insertedSections[indexPath.section] = makeSectionForInserting(at: indexPath.section)
        } else {
            insertedItems[indexPath] = makeItemForInserting()
        }
    }

    private func handleMoveAction(of update: UICollectionViewUpdateItem, state: ListLayoutState<Layout>) {
        guard let indexPath = update.indexPathBeforeUpdate else {
            fatalError("`indexPathBeforeUpdate` cannot be `nil` for a `move` update action")
        }
        ...
        guard let newIndexPath = update.indexPathAfterUpdate else {
            fatalError("`indexPathAfterUpdate` cannot be `nil` for a `move` update action")
        }
