...
public struct VerticalListLayout {
    ...
    public let metrics: VerticalListMetrics
    public let appearance: ListLayoutAppearance
    public let scrollAnchor: ListLayoutScrollAnchor
    ...
    private func resolveItemSize(
        ...
        let estimatedSize = CGSize(
            width: containerSize.width,
            height: metrics.estimatedHeight ?? 120.0
        )
...
extension VerticalListLayout: ListLayout {
    ...
    public func verticalScrollAnchorItem(
        ...
        for itemIndexPath in visibleItems {
            guard let item = state.item(at: itemIndexPath), let itemOrigin = item.origin else {
                continue
            }

            let itemSize = item.size?.value ?? .zero

            let itemPosition = CGPoint(
                x: itemOrigin.x + itemSize.width * scrollAnchor.y,
                y: itemOrigin.y + itemSize.height * scrollAnchor.y
            )
...
extension VerticalListLayout: Changeable {
    ...
    internal init(copy: ChangeableWrapper<Self>) {
        self.init(
            ...
            scrollAnchor: copy.scrollAnchor
        )
    }
