...
internal final class CollectionViewStateManager<Layout: FlowLayout>:
    NSObject,
...
extension CollectionViewStateManager {
    ...
    internal func updateCell(at indexPath: IndexPath) {
    ...
    internal func updateHeaderView(at indexPath: IndexPath) {
    ...
    internal func updateFooterView(at indexPath: IndexPath) {
        guard let section = state.section(at: indexPath) else {
            return assertionFailure("Inconsistent state of UICollectionView")
        }
    ...
    internal func updateState(sections: [FlowSection<Layout>], context: ComponentContext) {
